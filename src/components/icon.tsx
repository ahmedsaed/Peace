...
const CHROME_PATHS = {
  ...
  /**
   * "This record has a receipt", on a list row.
   *
   * A PAPERCLIP IS MOSTLY NEGATIVE SPACE, and its legibility is set by the gap
   * rather than by the ink. Rasterised at the sizes it is actually drawn at,
   * the hole closes up and the whole thing becomes a blob below about 13px —
   * which is why the attach BUTTON uses a folder instead, and why this is drawn
   * at 14 rather than matching the 12px subtitle beside it. Widening the strokes
   * to compensate makes it worse, not better: the gap is what carries the shape.
   *
   * Centred on the 24-box. The first version was offset four units to the right,
